...
void TBReckDay(int **arr, int row, THREADED) {
	...
	while (taxaOver < matrix.num_taxa) {
		childTree =                                                   \
         genNextEckDayTask(sourceTree, childTree, mMTPWL.taxa, mMTPWL.pos, TH); 
...
void correctEckDayIndices (struct subTask *sourceSubTask, THREADED) {
  ...
  sourceSubTask->root.lChildIndx = pop(TH);
  sourceSubTask->root.rChildIndx = pop(TH);
...
void TBRmaxMin(THREADED) {
	...
	while (taxaOver < matrix.num_taxa) {
		childTree =                                                  \
            genNextTask(sourceTree, childTree, mMTPWL.taxa, mMTPWL.pos, TH);
		freeTree(sourceTree, TH);
		sourceTree = childTree;
		mMTPWL = maxMini(sourceTree, TH);
		taxaOver += 1;
	...
	childTree=genNextTask(sourceTree,childTree,mMTPWL.taxa,mMTPWL.pos,TH);
	freeTree(sourceTree, TH);
	tbr(childTree, TH);
	if (childTree->costOfSubTask < bestCost) {
	  bestCost = childTree->costOfSubTask;
