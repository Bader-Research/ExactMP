...
int purdomsLowerBnd(struct taskNode *tNode, THREADED) {
  ...
  for (site = 0; site < matrix.num_pars_inf_sites; site++) { 
	...
	maskVal = 1;
		...
		added = 0;
			...
			remStatesORed = maskStateInPos(remStatesORed, maskPos, TH);
               	}
               	maskVal = maskVal << 1;
        }
  }
...
int maskStateInPos(int stateVector, int bitPos, THREADED) {
	int mask;
...
void showBits(int stateVector, THREADED) {
