...
#endif
                                        /* generate 1st case */
	childTree = genNextEckDayTask(sourceTree, childTree, taxaNum, 1, TH); 
#ifdef ECK_AND_DAY_MIN_TREE_VERBOSE
	printTree(childTree, 0, TH);
#endif
	prevLength = childTree->costOfSubTask;
	freeTree(childTree, TH);

	currMinPos = 1;
	currMinTaxa = taxaNum; 

	for (pos=2; pos < arraySize; pos++) { 
	 childTree = genNextEckDayTask(sourceTree,childTree, taxaNum, pos, TH); 
#ifdef ECK_AND_DAY_MIN_TREE_VERBOSE
...
void eckDay(THREADED) {
	...
	while (taxaOver < matrix.num_taxa) {
		...
		printf("\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~~~");
#endif
		childTree = genNextEckDayTask(sourceTree, childTree,         \
                                              mMTPWL.taxa, mMTPWL.pos, TH); 
		freeTree(sourceTree, TH);
  	...
  	childTree = genNextEckDayTask(sourceTree, childTree, mMTPWL.taxa,    \
                                      mMTPWL.pos, TH); 
...
struct subTask *genNextEckDayTask(struct subTask *currTask,                   \
                      struct subTask *nextTask, int taxa, int pos, THREADED) {
   ...
   setParentChildIndices(nextTask, TH);
