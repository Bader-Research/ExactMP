...
int **genIstOptSubTask(int **currSubTask, THREADED) {
  ...
  currSubTask[0][0] = skipSize+lhsOffSet;       
                                      // Size of RHS including 1
  currSubTask[0][1] = 3*skipSize+1; 
...
void convert(int *currTask, int socTask, int **nextTask, int taxa, int decompLevel, int *stack, THREADED) {
   int ctsl, ctsr, ntsl, ntsr, skipSize, lhsOffSet,
       ctr=0, sp=-1, hin=0, i, indx, pos, nctr, j,
       rci=0, lci=0, si=0, length=0, lsize, rsize,
       site=0, lc, rc, pa, flag, var, nflg;
   ...
   for (i=0; i < socTask; i++) {
     //printf("currTask[%d]: %d\n",i, currTask[i]);
     if (currTask[i]==-1) break;
   }
   lsize = i;
   rsize = socTask - i;

   //printf("lsize: %d, rsize: %d, socTask: %d, currTask[%d]: %d\n",lsize, rsize, socTask, i, currTask[i]);

				      // Get size of LHS tree
   ntsl = (i+1)*skipSize+lhsOffSet;
				      // Get size of RHS tree
   ntsr = (socTask-i)*skipSize+1;
				      // Get highest internal node
   hin = 1-socTask/2;
   /*
   printf("lsize: %d, rsize: %d, hin: %d\n",ntsl, ntsr, hin);
   getchar();
   */
   ...
   for (i=0; i < lsize; i++) {
     indx = lhsOffSet+(i+1)*skipSize;
     nextTask[0][indx] = currTask[i];
     if (currTask[i] > 0) {
                                      // copy states of newly added leaf node
         memcpy(&nextTask[0][indx+5], matrix.reord_sites_enc[currTask[i]-1],\
		                 matrix.num_pars_inf_sites*sizeof(int));
     }
   }

   for (i=lsize; i < socTask; i++) {
     indx = 1+(i-lsize)*skipSize;
     nextTask[1][indx] = currTask[i];
     if (currTask[i] > 0) {
                                      // copy states of newly added leaf node
         memcpy(&nextTask[1][indx+5], matrix.reord_sites_enc[currTask[i]-1],\
		                 matrix.num_pars_inf_sites*sizeof(int));
     }
   }
   ...
   printf("LHS: ");
   for (j=0; j < ntsl; j++) {
     printf("[%d] ",nextTask[0][j]);
   }
   printf("\n");

   printf("RHS: ");
   for (j=0; j < ntsr; j++) {
     printf("[%d] ",nextTask[1][j]);
   }
   getchar();
   */
  ...
  for (sp=-1, ctr=ntsr-skipSize; ctr > 0; ctr = ctr-skipSize) {
	 ...
	 stack[sp] = ctr;
         ...
         nextTask[1][ctr+4] = 0;
         for (length=0, site=0; site < matrix.num_pars_inf_sites; site++) {
            nextTask[1][ctr+5+site] = nextTask[1][nextTask[1][ctr+2]+site+5] & nextTask[1][nextTask[1][ctr+3]+5+site];
            if (nextTask[1][ctr+5+site]==0) {
              nextTask[1][ctr+5+site] = nextTask[1][nextTask[1][ctr+2]+site+5] | nextTask[1][nextTask[1][ctr+3]+5+site];
              ++length;
            }
         }
         nextTask[1][ctr+4] = nextTask[1][nextTask[1][ctr+2]+4] + nextTask[1][nextTask[1][ctr+3]+4] + length;
       }
  }
                                      // set parent child relationship for the
                                      // LHS child tree
  for (sp=-1,ctr=ntsl-skipSize;ctr>=lhsOffSet+skipSize;ctr=ctr-skipSize) {
       if (nextTask[0][ctr] > 0) {
         ...
         nextTask[0][ctr+4] = 0;
         for (length=0, site=0; site < matrix.num_pars_inf_sites; site++) {
            nextTask[0][ctr+5+site] = nextTask[0][nextTask[0][ctr+2]+site+5] & nextTask[0][nextTask[0][ctr+3]+5+site];
            if (nextTask[0][ctr+5+site]==0) {
              nextTask[0][ctr+5+site] = nextTask[0][nextTask[0][ctr+2]+site+5] | nextTask[0][nextTask[0][ctr+3]+5+site];
              ++length;
            }
         }
         nextTask[0][ctr+4] = nextTask[0][nextTask[0][ctr+2]+4] + nextTask[0][nextTask[0][ctr+3]+4] + length;
       }
  }
  ...
  printf("LHS: ");
  for (indx=0; indx < ntsl; indx++) {
    printf("[%d] ",nextTask[0][indx]);
  }
  printf("\n");

  printf("RHS: ");
  for (indx=0; indx < ntsr; indx++) {
     printf("[%d] ",nextTask[1][indx]);
  }
  getchar();*/
}
