...
void branchAndBound(THREADED) {
  ...
  while (taskSTop != NULL) {
    ...
    if ((currTaxa = dequeueTaxaQ(parentTaskNode, TH)) != -1) {
                                   /* For each possible position */
        for (pos=0; pos < numElements; pos++) { 
           if (parentTask->node[pos].id!= -1) { 
                                   /* Generate child task by adding taxon 
                                      in given position */
              childTask=genNextTask(parentTask, childTask, currTaxa, pos, TH);
	      ++checked;
    ...
    printTree(childTask, pos, TH);
                     ...
                     childTaskNode = copyTaxaQ(parentTaskNode, TH); 
		     ...
		     childTaskNode->taxaDoneTail->next = NULL;
	             ...
	             purdLowBnd = purdomsLowerBnd(childTaskNode, TH);	
  ...
  printBestSolutions(bestCost, checked, pruned, initBestCost, fp, TH);
