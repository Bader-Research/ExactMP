...
void setFrameWork(char *argv, THREADED) {
  ...
  if (MYTHREAD==0) {
     ...
     pthread_mutex_init(&(bclock), NULL);
     pthread_mutex_init(&(sqlock), NULL);
     solQCtr = -1;
     sqend = 944;
     ...
     sharedQ = (int ***)calloc(945, sizeof(int **));
     assert(sharedQ);
     for (i=0; i < 945; i++) {
       sharedQ[i] = (int **)calloc(2, sizeof(int *));
       assert(sharedQ[i]);
       sharedQ[i][0] = (int *)calloc((2*matrix.num_taxa*skipSize)+lhsOffSet,sizeof(int));
       assert(sharedQ[i][0]);
       sharedQ[i][1] = (int *)calloc((2*matrix.num_taxa*skipSize)+1,sizeof(int));
       assert(sharedQ[i][1]);
     }
