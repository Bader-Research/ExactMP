...
void printBestSolutions(int bc, int chk, int prn, int ibc, FILE *fp) {
...
#ifdef BnB_VERBOSE
		printTree(delNode->bCNode, -1);
#endif

		wr2File(delNode->bCNode, fp, ++bsNum);
...
void pushBestCostNode(struct subTask *addBCNode) {
	...
	if (bestCostStackHead == NULL) {
		bestCostStackHead = (struct bestCostStack *)malloc(sizeof(struct bestCostStack));
		bestCostStackHead->down = NULL;
		bestCostStackHead->bCNode= addBCNode;
	}
	else {
		newNode = (struct bestCostStack *)malloc(sizeof(struct bestCostStack));
...
void freeStackNode(struct taskNode *srcNode) {
   struct taxaQ *temp;
