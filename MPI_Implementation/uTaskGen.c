/*************************************************************************************************************************
 * DESCRIPTION: This function generates the array which contains the next tree encapsulated in preordered form.          *
 *                                                                                                                       * 
 * INPUTS:                                                                                                               *
 * (a) Partial Tree containing:                                                                                          *
 *                            (i) highest internal node in it,                                                           *
 *                            (ii) last taxon added,                                                                     *
 *                            (iii) tree number which is currently not implemented,                                      *
 *                            (iv) number of elements in the array containing preordered tree,                           *
 *                            (v) array itself,                                                                          *
 * (b) Taxon which is to be added                                                                                        *
 * (c) Position where the taxon is to be added                                                                           *
 *                                                                                                                       * 
 * WORKING: This function decrements internal node by 1 (internal node runs in negative values for identification) and   *
 * adds the {internal node, taxon} pair in position specified. Position 0 starts before the array, for example, in       *
 * {1,-1,2,3} it is before first element 1.                                                                              *
 *                                                                                                                       *
 * NOTE: The taxon should not be added before the internal node -1 to keep the order sustained. But, this check is not   *
 *       done here. The parent function should take care of it.                                                          *
 *************************************************************************************************************************/

#include "header.h"

struct subTask *genNextTask(struct subTask *currTask, struct subTask *nextTask, int taxa, int pos) { 

   int i;

   nextTask = (struct subTask *)calloc(1, sizeof(struct subTask));

                                                        /* set chld task's pos of int node "-1" ,this is rhs subtree root*/
                                                        /* if new node inserted after "-1" then pos of "-1" in child is  */
   ...
   computeCost(nextTask, currTask->rhtSubTreeRootIndx, pos);
   return (nextTask);

}

/*************************************************************************************************************************
 * DESCRIPTION: This function scans the whole array once and sets the parent child relationships. Doing this enables     *
 * computing cost of child tree from node to root without having to scan through all nodes always                        *
 *                                                                                                                       *
 * INPUTS: Sub task                                                                                                      *
 *                                                                                                                       *
 * OUTPUTS: None. The elements of subtask are updated                                                                    *
 ************************************************************************************************************************/
...
/*************************************************************************************************************************
 * DESCRIPTION: This function generates first subtask with 3 taxa. Very straightforward, no commenting needed            *
 *************************************************************************************************************************/
struct subTask *genIstSubTask(void) {
  ...
  currSubTask->node = (struct subTaskArrayNode *)calloc(4, sizeof(struct subTaskArrayNode ));
  ...
  currSubTask->node[0].stateVector = (int *)calloc(matrix.num_pars_inf_sites, sizeof (int));
  memcpy(currSubTask->node[0].stateVector,matrix.reord_sites_enc[0],matrix.num_pars_inf_sites*sizeof(int));
  ...
  currSubTask->node[2].stateVector = (int *)calloc(matrix.num_pars_inf_sites, sizeof (int));
  memcpy(currSubTask->node[2].stateVector,matrix.reord_sites_enc[1],matrix.num_pars_inf_sites*sizeof(int));
  ...
  currSubTask->node[3].stateVector = (int *)calloc(matrix.num_pars_inf_sites, sizeof (int));
  memcpy(currSubTask->node[3].stateVector,matrix.reord_sites_enc[2],matrix.num_pars_inf_sites*sizeof(int));
  ...
  currSubTask->node[1].stateVector = (int *)calloc(matrix.num_pars_inf_sites, sizeof (int));
  performFitchOp(&leftChild, &currSubTask->node[1], &rightChild);
  ...
  currSubTask->root.stateVector = (int *)calloc(matrix.num_pars_inf_sites, sizeof (int));
  performFitchOp(&leftChild, &currSubTask->root, &rightChild);
